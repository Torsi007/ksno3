...
import ksno.service.WorkTaskServiceImpl;
 ...
 * @author tor.hauge
 */
public class WorkTaskConverter implements Converter {

    private Logger getLogService(){
      return Logger.getLogger(WorkTaskConverter.class.getName());
    }

    private WorkTaskService getWorkTaskService(){
        WorkTaskService service = null;
        try {
            service = (WorkTaskService) JSFUtil.getBeanValue("#{WorkTaskService}", WorkTaskService.class);
            //return (ArticleCategoryService)FacesContextUtils.getWebApplicationContext(context).getBean("articleCategoryService");
        } catch (Exception ex) {
            Logger.getLogger(WorkTaskConverter.class.getName()).log(Level.SEVERE, null, ex);
        }
        return service;
    }
    ...
    public Object getAsObject(FacesContext context, UIComponent component, String value) throws ConverterException {
        ...
        return getWorkTaskService().getWorkTask(Long.parseLong(value));
    }
